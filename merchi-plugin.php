...
function gc_load_background_classes_on_admin_init()
{
    ...
    include_once __DIR__ . '/class-wp-example-request.php';
...
add_filter(
    'woocommerce_is_purchasable', function ($res, $obj) {
...
function enqueue_admin_customfiles()
{
    ...
    wp_localize_script(
        'custom-admin-script', 'scriptData', array(
...
function enqueue_my_public_script()
{
    ...
    wp_localize_script(
        'custom-public-script', 'scriptData', array(
...
{
    if ($post->post_type == 'product' && (isset($_POST['hidden_product_name'])) && !empty($_POST['hidden_product_name'])) {
        if (isset($_POST['hidden_product_id']) && !empty($_POST['hidden_product_id'])) {
...
function bbloomer_alter_price_cart( $cart )
{
    ...
    //LOOP THROUGH CART ITEMS & APPLY ITEM PRICING
    ...
    class WC_Merchi_Gateway extends WC_Payment_Gateway
    {
      ...
      <div id="payment-message" class="hidden"></div>
        ...
        public function payment_scripts()
        {
            ...
            wp_localize_script(
                'custom-public-script', 'merchi_params', array(
