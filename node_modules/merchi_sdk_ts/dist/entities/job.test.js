...
test('Job deadline serialised to milliseconds in form data', function () {
    ...
    var correct = [['deadline', '1362096000']];
    var backData = Array.from(job.toFormData().entries());
    expect(backData).toEqual(correct);
});
test('Job deadline serialised to milliseconds in JSON', function () {
    var merchi = new Merchi();
    var job = new merchi.Job();
    job.deadline = new Date('Feb 28 2013 19:00:00 GMT-0500');
...
test('Job deadline serialised from milliseconds in JSON', function () {
    var merchi = new Merchi();
    var job = new merchi.Job().fromJson({ deadline: 1362096000 });
    expect(job.deadline).toEqual(new Date('Feb 28 2013 19:00:00 GMT-0500'));
});
test('add two files to job', function () {
    var merchi = new Merchi();
    var job = new merchi.Job();
    var f1 = new merchi.MerchiFile();
    f1.fromFormFile(new File([''], '1'));
    var f2 = new merchi.MerchiFile();
    f2.fromFormFile(new File([''], '2'));
    job.clientFiles = [f1, f2];
    var fetch = mockFetch(true, { 'job': {} }, 200);
    return job.save().then(function () {
        var data = Array.from(fetch.mock.calls[0][1]['body'].entries());
...
test('Get quote update job cost', function () {
    var merchi = new Merchi();
    var jobWithExistingProduct = new merchi.Job().fromJson({ 'quantity': 10, 'cost': 0, product: { 'id': 1, 'name': 'test' } });
    var jobWithNewProduct = new merchi.Job().fromJson({ 'quantity': 10, 'cost': 0, product: { 'name': 'test' } });
    mockFetch(true, { 'quantity': 10, 'cost': 100 }, 200);
    jobWithExistingProduct.getQuote().then(function (job) {
        expect(job.quantity).toEqual(10);
        expect(job.cost).toEqual(100);
    });
    jobWithNewProduct.getQuote().then(function (job) {
        expect(job.quantity).toEqual(10);
        expect(job.cost).toEqual(100);
    });
});
test('test deduct job inventory', function () {
    var merchi = new Merchi();
    var job = new merchi.Job();
    var inventory = new merchi.Inventory();
    inventory.id = 1;
    job.id = 1;
    var matchingInventoryOne = new merchi.MatchingInventory();
...
test('test bulk deduct job without inventories will cause error', function () {
    var merchi = new Merchi();
    var job = new merchi.Job();
