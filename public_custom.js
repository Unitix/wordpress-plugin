...
async function patchRecieverAddress(cart, address, step){
	...
	},(status, data) => console.log(`Error ${status}: ${data}`), undefined, 5 );
}
...
async function addClientToCart(cart, userId) {
	...
	const user = new MERCHI.User().id(userId);
		const cartEnt = await localStorageGetCartEnt();
		console.log('cartEntID', cartEnt.id());
		console.log('cartEntToken', cartEnt.token());
		console.log('cartEntClient', cartEnt.client());
		cart.client(user)
		cart.patch((response) => {
			localStorageUpdateCartEnt(response);
		},(status, data) => console.log(`Error ${status}: ${data}`), embed );
}
...
async function updateShipmentMethod(index, quoteIndex){
	...
	const embed = {
		...
		cart.get((cartEnt)=>{
			...
			},(error) => console.log(JSON.stringify(error)), embed);
		},(error) => console.log(JSON.stringify(error)), embed);
}

function navigateStep(step) {
	jQuery('.checkout-navigation .button').attr('disabled', 'disabled');
	if(document.getElementById('captured_email') && '' !== document.getElementById('captured_email').value){
		...
		for (var i = 0; i < requiredElems.length; i++) {
			if( requiredElems.item(i) ){
				...
				var actualInputElem = requiredElems.item(i).getElementsByClassName('woocommerce-input-wrapper')[0].getElementsByClassName("input-text")[0];
				var actualSelectElem = requiredElems.item(i).getElementsByClassName('woocommerce-input-wrapper')[0].getElementsByClassName("select2-hidden-accessible")[0];
				if( actualInputElem && !actualInputElem.value ){
					noticeStrong.innerHTML = requiredElems.item(i).getElementsByClassName('woocommerce-input-wrapper')[0].getElementsByClassName("input-text")[0].getAttribute("data-cstname");
					noticeLi.append(' is a required field');
					noticeUl.append(noticeLi);
					error = true;
				}else if( actualSelectElem && !actualSelectElem.value ){
					noticeStrong.innerHTML = requiredElems.item(i).getElementsByClassName('woocommerce-input-wrapper')[0].getElementsByClassName("select2-hidden-accessible")[0].getAttribute("data-cstname");
					noticeLi.append(' is a required field');
					noticeUl.append(noticeLi);
					error = true;
				}
		...
		if(error){
			...
			var billing_address_1 = document.getElementById('billing_address_1') ? document.getElementById('billing_address_1').value : false;
			var billing_address_2 = document.getElementById('billing_address_2') ? document.getElementById('billing_address_2').value : false;
			var billing_city = document.getElementById('billing_city') ? document.getElementById('billing_city').value : false;
			var billing_state = document.getElementById('billing_state') ? document.getElementById('billing_state').value : false;
			var country = document.getElementById('billing_country') ? document.getElementById('billing_country').value : false;
			var postcode = document.getElementById('billing_postcode') ? document.getElementById('billing_postcode').value : false;
			var phone = document.getElementById('billing_phone') ? document.getElementById('billing_phone').value : false;
			var fname = document.getElementById('billing_first_name') ? document.getElementById('billing_first_name').value : false;
			const cookieValue = getCookieByName('cart-'+scriptData.merchi_domain);
			var token = false;
			var id = false;
			if (cookieValue) {
...
function setCookie(name, value, days) {
	...
	jQuery.post( ajax_url, ajax_data, function( response ) {
	  var expires = "";
		if (days) {
			console.log('Cookie is getting created');
			var date = new Date();
...
function getButtonByText(buttons, text) {
  for (let i = 0; i < buttons.length; i++) {
	  console.log('INHM', buttons[i].innerHTML);
...
jQuery(document).ready(function ($) {
	...
	$( document.body ).on( 'updated_checkout', function(data) {
		...
		$.post( ajax_url, ajax_data, function( response ) { 
			...
			$("#billing_phone").keydown(function(e) {
				var oldvalue= frontendajax.telephoneInput ? frontendajax.telephoneInput : $(this).val();
				var field=this;
				setTimeout(function () {
	...
	document.addEventListener('click', function(event) {
		  ...
		  if (target.classList.contains('btn-primary') && !target.classList.contains('product-group-button-add')) {
			...
			setTimeout(function(){
					if (scriptData.is_single_product) {
						...
						cartEnt.get(
							(cart) => {
								console.log('cartEnt', cart);
								localStorageUpdateCartEnt(cart);
								const cartJson = new MERCHI.toJson(cart);
								console.log("Whole Cart :", cartJson);
								console.log("cart.item.id :", cartJson.id);
								console.log("cart.item.taxAmount :", cartJson.taxAmount);
								var cartPayload = {};
								...
								cartJson.cartItems.forEach(function(item, itemIndex){
									...
									item.variationsGroups.forEach(function(group, gi){
										...
										group.variations.forEach(function(variation, vi){
											...
											console.log('Variation Index', vi);
										...
										cartPayload['cartItems'][itemIndex]['objExtras'].push(objExtras);
									});
									
								});
								console.log('Cart Payload: ', cartPayload);
								if (cartJson.hasOwnProperty('cartItems') && Array.isArray(cartJson.cartItems) && cartJson.cartItems.length !== 0) {
									jQuery.ajax({
										method: 'POST',
										url: frontendajax.ajaxurl,
										data: {
										...
										success: function (response) {
											target.parentElement.classList.remove('cst-disabled-btn-parent');
								...
								//emptyCookie('cart-'+scriptData.merchi_domain);
				...
				console.log('This is not is_single_product.');
				}
			}, 3500);
	...
	jQuery( document ).on( 'click', '.remove.remove-product', function ( e ) {
		...
		jQuery.ajax( {
			...
			success: function ( response ) {
				if( 1 == jQuery('.cst_cart_item').length || 0 == jQuery('.cst_cart_item').length ){
				...
				cart.get((data) => {
					...
					console.log('DCartItems', ccart.cartItems());
					console.log('actual_pos', actual_pos);
	...
	jQuery('#cst_returning_customer').change(function() {
		...
		if (jQuery(this).is(':checked')) {
		  document.cookie = 'cstReturningUser=true; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/';
		} else {
		  document.cookie = 'cstReturningUser=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/';
		}
		location.reload();
	});
