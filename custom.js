    ...
    function attachfeatureMedia(imageUrl,inputString, msg) {
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                ...
                console.log("Attachment not created.");
            }
            },
            error: function(error) {
...
jQuery(document).ready(function($) {	
	...
	jQuery(document).on('click', function(event) {
		...
		if (!searchResults.is(event.target) && !customValueField.is(event.target) && searchResults.has(event.target).length === 0) {
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					if (data && data.product) {
						...
						if (product_title) {
							console.log("javascript console => ", product_title);
						...
						if (featureImage && featureImage.downloadUrl && featureImage.mimetype) {
							const inputString = featureImage.mimetype;
							const parts = inputString.split('/');
							const fileType = parts[1];
							console.log(fileType);
							console.log("Feature Image URL:", featureImage.id);
						...
						if (Array.isArray(imagesArray)) {
							for (const image of imagesArray) {
								if (image && image.downloadUrl && image.mimetype) {
									...
									console.log(fileType);
									console.log(fileType);
									console.log("Image URL:", image.id);
							...
							console.log("Images array not available.");
						}
					} else {
